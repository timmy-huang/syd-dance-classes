...
def kcc(callback, start_date, _id):
    """
    Fetch KCC schedule data and sync to API.
    ...
    # Scrape classes
    classes = scrape_kcc_classes(callback, start_date, _id)
    ...
    # Sync to API
    result = api_client.sync_classes("kcc", transformed)
    return result


def scrape_kcc_classes(callback, start_date, _id):
    """
    Scrape KCC classes from MindBody widget
    ...
    try:
        ...
        if response.status_code != 200:
            print(f"⚠️  KCC request failed with status code {response.status_code}")
            return []
        ...
        print(f"✓ Scraped {len(data)} classes from KCC")
        return data
        
    except requests.exceptions.RequestException as e:
        print(f"❌ Error scraping KCC: {e}")
        return []
    except Exception as e:
        print(f"❌ Error parsing KCC data: {e}")
        return []
...
if __name__ == "__main__":
    ...
    callback = "jQuery36407072146344659085_1747063164353"
    _id = "1747063164355"
    
    result = kcc(callback, date.today(), _id)
