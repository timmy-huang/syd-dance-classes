...
# Widget URL for Sydney Dance Company schedule
WIDGET_URL = "https://brandedweb-next.mindbodyonline.com/components/widgets/schedules/view/e89579590/schedule"
        ...
        if time_match:
            ...
            for j in range(i + 1, min(i + 8, len(lines))):
                ...
                for ct in class_types:
                    if next_line.upper().startswith(ct):
                ...
                if re.match(r'^\d{1,2}:\d{2}\s*(?:AM|PM)$', next_line, re.IGNORECASE):
                    break
                if next_line.lower() in ['show details', 'sign up', 'unavailable', 'sydney dance company']:
                    break
            ...
            if class_name:
                ...
                if start_dt:
                    ...
                    service_id = f"sdc_{current_date.isoformat()}_{time_str.replace(' ', '').replace(':', '')}_{class_name.replace(' ', '_')[:20]}"
...
async def scrape_sdc_classes_async(start_date, end_date):
    """
    Scrape Sydney Dance Company classes using Playwright headless browser.
    """
    all_classes = []
    ...
    async with async_playwright() as p:
        ...
        try:
            print(f"[INFO] Loading Sydney Dance Company schedule...")
    ...
    print(f"[OK] Scraped {len(all_classes)} classes from Sydney Dance Company")
    return all_classes


def scrape_sdc_classes(start_date, end_date):
    """Synchronous wrapper for the async scraping function."""
    return asyncio.run(scrape_sdc_classes_async(start_date, end_date))
